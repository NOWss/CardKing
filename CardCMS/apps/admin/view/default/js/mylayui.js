layui.use(['element', 'upload', 'laydate', 'form'], function () {
  ...
  form.on('submit(login-submit)', function (data) {
    ...
    const checkcode = form.find("#checkcode").val();
    const formcheck = form.find("#formcheck").val();
    const qrcode = form.find("#qrcode").val();
    $.ajax({
      type: 'POST',
      url: url,
      dataType: 'json',
  ...
  form.on('submit(authener-submit)', function (data) {
    const form = $("#doQrCode");
    const url = form.attr('action');
    const validCode = form.find("#validCode").val();
    const formcheck = form.find("#formcheck").val();
    $.ajax({
      type: 'POST',
      url: url,
      ...
      data: {
        ...
        if (response.code == 1) {
          ...
          form.find("#validCode").val("");
          layer.msg("验证失败：" + response.data, {icon: 5});
        }
      }, error: function (xhr, status, error) {
        layer.msg("验证请求发生错误!", {icon: 5});
        $('#note').html('验证请求发生错误，您可按照如下方式排查：<br>1、试着删除根目录下runtime目录，刷新页面重试;<br>2、检查系统会话文件存储目录是否具有写入权限；<br>3、检查服务器环境pathinfo及伪静态规则配置；<br>4、如果还不行那就到交流群出钱请人处理吧！');
      }
    })
    return false;
  })
  ...
  var uploadInst = upload.render({
    ...
    , acceptMime: 'image/*', before: function (obj) {
      //判断是否需要加水印
  ...
  var uploadsInst = upload.render({
    ...
    , acceptMime: 'image/*', before: function (obj) {
      ...
      if (res.code == 1) {
        ...
        html += "<dl><dt><img src='" + sitedir + res.data[0] + "' data-url='" + res.data[0] + "'></dt><dd>删除</dd>" + "<dt><input type='text' name='picstitle[]' style='width:95%' /></dt>" + "</dl>";
